...
| 서비스 | 주소 |
|---|---|
| 백엔드 (roamit-api) | http://localhost:3001 |
| 프론트엔드 (roamit) | http://localhost:3000 |

프론트엔드는 Next.js 기본 포트(3000)를 그대로 쓰고, 백엔드가 겹치지 않도록 3001 포트로 실행됩니다.

백엔드는 `CORS_ORIGIN` 환경변수에 등록된 origin만 허용하고(`.env.example` 기본값: `http://localhost:3000`), 목록에 없는 origin의 요청은 차단합니다. 쿠키 등 credentials 를 포함한 요청을 허용하려면 `CORS_CREDENTIALS=true`로 설정하세요. 배포 환경에서는 실제 프론트엔드 도메인으로 `CORS_ORIGIN`을 교체합니다.
